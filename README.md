...
## 📖 Tài liệu Giao diện (UI Documentation)

Toàn bộ các UI Components trong `src/components` đã được viết tài liệu kỹ thuật chi tiết bằng TSDoc. Để xuất và xem tài liệu:

1. Chạy lệnh xuất HTML:
   ```bash
   npm run docs
   ```
2. Chạy server tĩnh để xem (yêu cầu npx):

```bash
   npx serve docs
```

3. Truy cập vào địa chỉ `http://localhost:3000` (hoặc cổng hiển thị trên Terminal) để xem danh sách Components cùng cách sử dụng (`@example`).
